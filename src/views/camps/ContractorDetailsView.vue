<template>
  <div class="ComponentWrapper">
    <div class="MGroup">
      ...
      <MComboBox ref='ContractorWorkPlace' :Name="'ContractorWorkPlace'" :Label="'موقع العمل'" :Items="ContractorWorkPlaceItems"></MComboBox>
    ...
    <div class="MGroup" id="CampsGroup">
      ...
      <MComboBox ref='AvailableCamps' :Name="'AvailableCamps'" :Label="'الكمب'" :Clearable="true"
        :ItemsName="'camp_full_name'" :Items="AvailableCampsItems"></MComboBox>
      <div class="MGroupButtons">
        <div class="MButton" id="AddCamp" v-show="AvailableCamps && AvailableCamps.Get().length > 0">اضافة</div>
      </div>
      <MTable ref="CampsTB" :Name="'CampsTB'" :DataArray="CampsTBData" :Columns="CampsTBColumns" :Sums="CampsTBSums"
        ...
        <template v-slot:options>
          <div class="MTableOption" OptionEventName="DeleteItem">
            <div class="MTableOptionIcon"><svg viewBox="0 0 1000 1000">
                <path d="M871.4,190c0.4,13.1-8.9,27.4-21.9,40.3c-85.2,84.9-170.2,170.1-255.3,255.2c-14.5,14.5-14.5,14.5,0.4,29.4
	c85.3,85.3,170.7,170.8,256.1,256c15.3,15.2,23.9,32.4,20,54.5c-7.5,41.4-55.6,60.3-89.1,34.7c-4.8-3.7-9.2-8.2-13.6-12.5
	c-86.1-86-172.1-171.8-257.8-258.2c-8.3-8.4-12.3-8.1-20.4,0.1C403,677,315.5,764,228.4,851.4c-17.3,17.4-37.2,25-61.2,17.6
	c-36.2-11.2-50.6-55.5-28.3-86.1c3.6-4.9,8.1-9.3,12.4-13.6c85.6-85.7,171.3-171.4,257-257.1c12.1-12.1,12.1-12.1-0.4-24.7
	c-86.2-86.3-172.5-172.7-259-258.9c-17.8-17.7-25.6-37.8-17.5-62.3c13.2-39.8,62.3-50.9,93.4-20.9c29.8,28.8,58.7,58.4,88,87.7
	c59.2,59.2,118.5,118.3,177.4,177.7c7.7,7.7,11.6,8.1,19.4,0.2c87-87.5,174.6-174.4,261.8-261.7c12-12.1,25.3-20.2,42.8-20.5
	C846.1,128.4,871,152.4,871.4,190z" />
              </svg></div>
...
export default {
  setup() {
    return {
      ...
      AvailableCamps: ref(null),
      AvailableCampsItems: ref([]),
      EntryDate: ref(null),
      Camps: ref([]),
      CampsTB: ref(null),
      CampsTBData: ref([]),
  ...
  mounted() {
    ...
    document.getElementById('AddCamp').addEventListener('click', function () {
      if (Instance.AvailableCamps.Get().length == 0) {
        return;
      }

      let Existed = false;
      Instance.Camps.forEach(Camp => {
        if (Camp['id'] == Instance.AvailableCamps.Get()[0]['id']) {
          if (Camp['ActionType'] == 'DELETE') {
            Camp['ActionType'] = '';
            Existed = true;
          } else {
            window.ShowMessage('الكمب مضاف مسبقا');
            return;
          }
        }
      });
      ...
      if (Existed == false) {
        let CampData = {};
        CampData['id'] = Instance.AvailableCamps.Get()[0]['id'];
        CampData['contractor_id'] = Instance.ID;
        CampData['camp_name'] = Instance.AvailableCamps.Get()[0]['camp_name'];
        CampData['room_no'] = Instance.AvailableCamps.Get()[0]['room_no'];
        CampData['ActionType'] = 'ADD';
        Instance.Camps.push(CampData);
      }

      Instance.AvailableCamps.Clear();
    ...
    document.getElementById('CampsTB').addEventListener('DeleteItem', function (data) {
      for (let i = Instance.Camps.length - 1; i >= 0; i--) {
        if (Instance.Camps[i]['id'] == data.detail.RowData['id']) {
          if (Instance.Camps[i]['ActionType'] === 'ADD') {
            Instance.Camps.splice(i, 1);
          } else {
            Instance.Camps[i]['ActionType'] = 'DELETE';
          }
          break;
        }
      }
  ...
  methods: {
    ...
    Save() {
      ...
      Parameters.append('contractor_work_place', this.ContractorWorkPlace.Get());
    ...
    GetAvailableCamps() {
      window.ShowLoading();
      api.get('GetAvailableCamps').then(response => {
        if (response.data) {
          window.HideLoading();
          this.AvailableCampsItems = [...response.data, ...this.Camps];
        } else {
          this.AvailableCampsItems = [];
          window.HideLoading();
          window.ShowMessage(response.data);
        }
      }).catch(() => {
