<template>
  <div class="ComponentWrapper">
    <RouterLink to="/camps/add" class="MButton" id="AddCampBTN">اضافة كمب</RouterLink>
    <div class="MButton" id="ReloadCampsBTN">اعادة تحميل البيانات</div>
    <MTable ref="CampsTB" :Name="'CampsTB'" :DataArray="CampsTBData" :Columns="CampsTBColumns" :Sums="CampsTBSums"
      :GetDataFunction="GetCamps" :RowsCount="CampsTBRowsCount" :RowsPerPage="10">
    ...
    return {
      ...
      CampsTBColumns: [
        ...
        },
        {
          ...
          label: 'اسم الكمب',
          filter: 'combo',
          filter_items: GlobalsStore.value.ComboBoxes?.Camps || [],
        },
        {
          name: 'room_no',
          label: 'رقم الغرفة',
      ...
      CampsTBSums: ref([]),
      CampsTBRowsCount: ref(0),
    };
  },
  mounted() {
    this.CampsTB.LoadMTable();
    let Instance = this;
    ...
    document.getElementById('CampsTB').addEventListener('ViewItem', function (data) {
      ...
      Instance.$router.push({ name: 'EditCamp' });
    });
  ...
  methods: {
    GetCamps(MTable) {
      api.get('GetCamps', {
        ...
        this.CampsTBData = response.data.data
        this.CampsTBRowsCount = response.data.total
        this.CampsTBSums = response.data.sums
      }).catch(error => {
