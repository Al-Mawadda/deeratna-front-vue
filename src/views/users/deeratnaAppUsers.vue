<template>
  <div class="ComponentWrapper">
    <!-- ===============  add subsc Model  ========= -->
    <MModal ref="AddCustomerModal" :Name="'AddCustomerModal'" :Title="'اضافة مشترك جديد'">
      <!-- <MComboBox
        ref="Compounds"
        :Name="'Compounds'"
        :Label="' اسم المدينة *'"
    ...
    <div class="MGroup">
      <MDate ref="CustomersFromDate" :Name="'CustomersFromDate'" :Label="'تاريخ من'"></MDate>
      <MDate ref="CustomersToDate" :Name="'CustomersToDate'" :Label="'تاريخ الى'"></MDate>
    </div>
    <MTable ref="CustomersTB" :Name="'CustomersTB'" :DataArray="CustomersTBData" :Columns="CustomersTBColumns" :Sums="CustomersTBSums" :GetDataFunction="GetCustomersData" :RowsCount="CustomersTBRowsCount" :RowsPerPage="10">
  ...
  c-31-0.1-56.9,25.8-56.8,56.8C443.8,530.8,469.7,556.6,500.2,556.5z"
        ...
        <div class="MTableOption" OptionEventName="DeleteIdDevice">
          <div class="MTableOptionIcon">
            <svg viewBox="0 0 1000 1000">
              <path
                d="M500.2,249.6c124.1,1.1,233.2,42.7,328.2,122.1c39.6,33,72.3,72.7,106.8,110.6c9,9.9,8.7,25.5-0.8,36.4
  ...
  c-31-0.1-56.9,25.8-56.8,56.8C443.8,530.8,469.7,556.6,500.2,556.5z"
          ...
          <div class="MTableOptionName">الغاء تفعيل</div>
        </div>
        <!-- <div class="MTableOption" OptionEventName="SendNotification">
          <div class="MTableOptionIcon">
            <svg viewBox="0 0 1000 1000">
              <path
                d="M500.2,249.6c124.1,1.1,233.2,42.7,328.2,122.1c39.6,33,72.3,72.7,106.8,110.6c9,9.9,8.7,25.5-0.8,36.4
  ...
  c-31-0.1-56.9,25.8-56.8,56.8C443.8,530.8,469.7,556.6,500.2,556.5z"
          ...
          <div class="MTableOptionName">ارسال اشعار</div>
        </div> -->
      </template>
    </MTable>
  </div>
  ...
  mounted() {
    ...
    document.getElementById('CustomersTB').addEventListener(
      ...
      function (data) {
        ...
        document.getElementById('username').querySelector('input').value = this.selectedRowData.username
        document.getElementById('phone').querySelector('input').value = this.selectedRowData.phone
        document.getElementById('status').querySelector('input').value = this.selectedRowData.status
    ...
    document.getElementById('CustomersTB').addEventListener(
      ...
      function (data) {
        ...
        this.ID = this.selectedRowData.id
        api
          .put(`ReactivateCustomer/` + this.ID)
          .then(response => {
            HideLoading()
            if (response.data.success) {
              ShowMessage(response.data.message)
    ...
    //   function (data) {
    //     this.selectedRowData = data.detail.RowData
    //     this.ID = this.selectedRowData.uid
    //     api
    //       .post(`SendPersonalNotification/` + this.ID)
    //       .then(response => {
    //         HideLoading()
    //         if (response.data.success) {
    //           ShowMessage(response.data.message)
